...
contract('TokenConverter', function (accounts) {
    ...
    it('User2 input CFNX and get 1/6 FNX', async function () {
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let beforeCFnxBalanceUser = await CFNXInst.balanceOf(accounts[2]);
        console.log(beforeCFnxBalanceUser);
        console.log(await CFNXInst.allowance(accounts[2],CvntProxyInst.address));

        let beforeFnxBalanceProxy = await CFNXInst.balanceOf(CvntProxyInst.address);
        let tx = await CvntProxyInst.inputCfnxForInstallmentPay(cfnxAmount1,{from:accounts[2]});
        assert.equal(tx.receipt.status,true);

        let afterFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let afterCFnxBalanceUser = await CFNXInst.balanceOf(accounts[2]);
        let afterFnxBalanceProxy = await CFNXInst.balanceOf(CvntProxyInst.address);
        ...
        let lockedBalance = await CvntProxyInst.lockedBalanceOf(accounts[2]);
        console.log(lockedBalance);
        assert.equal(web3.utils.fromWei(lockedBalance),web3.utils.fromWei(cfnxAmount1)*5/6);
    })

    it('Time not arriave,user1 try to Get FNX,but get 0 fnx', async function () {
        await time.increase(0.2*ONE_MONTH + 1);
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[1]);
        ...
        let afterFnxUser =  await FNXInst.balanceOf(accounts[1]);
        let diffFNXUser = web3.utils.fromWei(new BN(afterFnxUser)) - web3.utils.fromWei(new BN(beforeFnxUser));
        console.log("try get FNX diff user:" + diffFNXUser);
        assert.equal(diffFNXUser,0);
        let lockedBalance = await CvntProxyInst.lockedBalanceOf(accounts[1]);
        assert.equal(web3.utils.fromWei(lockedBalance),web3.utils.fromWei(cfnxAmount1)*5/6);
    })

    it('Time not arriave,user2 try to Get FNX,but get 0 fnx', async function () {
        await time.increase(0.7*ONE_MONTH + 1);
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let tx = await CvntProxyInst.claimFnxExpiredReward({from:accounts[2]});
        assert.equal(tx.receipt.status,true);
        await utils.sleep(1000);
        let afterFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let diffFNXUser = web3.utils.fromWei(new BN(afterFnxUser)) - web3.utils.fromWei(new BN(beforeFnxUser));
    ...
    it('user1 Get FNX 2/6,3/6 part', async function () {
        await time.increase(1.1*ONE_MONTH + 1);
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[1]);
        let tx = await CvntProxyInst.claimFnxExpiredReward({from:accounts[1]});
        assert.equal(tx.receipt.status,true);
        await utils.sleep(1000);
    ...
    it('user2 Get FNX 2/6,3/6 part', async function () {
        await time.increase(0.1*ONE_MONTH + 1);
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let tx = await CvntProxyInst.claimFnxExpiredReward({from:accounts[2]});
        assert.equal(tx.receipt.status,true);
        await utils.sleep(1000);
    ...
    it('User1 Get FNX 4/6,5/6,6/6 part', async function () {
    ...
    it('User2 Get FNX 4/6,5/6,6/6 part', async function () {
        await time.increase(4*ONE_MONTH + 1);
        let beforeFnxUser =  await FNXInst.balanceOf(accounts[2]);
        let tx = await CvntProxyInst.claimFnxExpiredReward({from:accounts[2]});
        assert.equal(tx.receipt.status,true);
