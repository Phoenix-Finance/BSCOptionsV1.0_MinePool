...
contract('TokenConverter', function (accounts) {
    ...
    it('1 User1 input CFNX with 100 tx and get 1/6 FNX ', async function () {
        ...
        for(var i=0;i<100;i++) {
            tx = await CvntProxyInst.inputCfnxForInstallmentPay(cfnxAmount1.div(new BN(100)),{from:accounts[1]});
            assert.equal(tx.receipt.status,true);
            console.log(i);
        }

        STARTINPUTTIME = time.latest();
        ...
        console.log(web3.utils.fromWei(lockedBalance));
        assert.equal(web3.utils.fromWei(lockedBalance),web3.utils.fromWei(cfnxAmount1)*5/6);

    })


    it('2 Get all FNX of the first batch input', async function () {
    ...
    it('3 User1 input CFNX in 100 tx and get 1/6 FNX after one day', async function () {
        ...
        for(var i=0;i<200;i++) {
            tx = await CvntProxyInst.inputCfnxForInstallmentPay(cfnxAmount1.div(new BN(200)),{from:accounts[1]});
            assert.equal(tx.receipt.status,true);
            console.log(i);
        }
